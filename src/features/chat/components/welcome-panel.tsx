    ...
    <div className="flex flex-1 flex-col items-center justify-center px-4 py-8 md:px-8">
      <div className="mx-auto w-full max-w-2xl space-y-8 text-center">
        ...
        <div className="space-y-4">
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-2xl ghost-glow ghost-border bg-gradient-to-br from-primary/15 to-teal-500/10">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {CAPABILITIES.map((cap) => {
            ...
            return (
              <div
                key={cap.title}
                className="rounded-xl border border-border/60 bg-card p-4 text-left shadow-sm transition-colors hover:border-primary/30"
              >
        ...
        <div className="flex items-center justify-center gap-4 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <Mic className={cn("h-3.5 w-3.5", voiceSttAvailable && "text-primary")} />
            语音输入
          </span>
    ...
    <div className={cn("flex gap-2 overflow-x-auto pb-1", className)}>
      {DEMO_PROMPTS.slice(0, 3).map((prompt) => (
        <Button
          ...
          variant="outline"
          size="sm"
          className="shrink-0 border-border/60 bg-secondary/30 text-xs hover:border-primary/40 hover:bg-primary/5"
